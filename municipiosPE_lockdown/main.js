...
function updateBoundaries() {
	...
	if (showBoundaries) {
		...
		if (option == 'cluster') {
			...
			let aplScale = d3.scaleOrdinal().domain(["nenhum", "Laticínios", "Gesso", "Confeccao", "Vitivinicultura"]).range(['#7fc97f', '#beaed4', '#fdc086', '#ffff99', '#386cb0']);
			clusters.forEach(cluster => {
				cluster.nodes.forEach(n => {
					let node = nodes[n];
...
function loadInterface() {
	...
	for (let key in layerBoundaries._layers) {
		...
		thisname = item.feature.properties.name.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase()
		if (thisname in nodes) {
			nodes[thisname].boundary = item;
			item.addTo(map);
			let center = item.getCenter();
			nodes[thisname].lat = center.lat;
			nodes[thisname].lng = center.lng;
			nodeMarkers[thisname].setLatLng(center);
			item.removeFrom(map);
		}
		else
...
function buildCoords() {
	...
	graph.forEach(node => {
		...
		nodes[node.name] = {
			...
			"salMedFor": node["salmed"],
			"idhm": node["idhm"],
			"unidSus": +node["undsus"],
			"popOcup": +node["popocupd"],
			"pibPC": +node["PIB per capita"],
			"popRendNom": +node["rendnom"],
			"apl": node["APL"],
			// "idhme": node["IDHM_E"],
			// "idhml": node["IDHM_L"],
			// "idhmr": node["IDHM_R"],
